#!/usr/bin/env python3
"""
Agent C — Autonomous agent for AgentMesh (Gemini-powered agentic loop).
Discovers and hires tools from ALL peers on the mesh, paying via x402.

Runs on Node C (AXL api_port 9022).

Setup:
  export AGENT_C_EVM_PRIVATE_KEY="0x..."   # wallet with USDC on Base Sepolia
  export GEMINI_API_KEY="..."              # Gemini API key for LLM reasoning

Run:
  python3 agents/agent-c/agent.py
  python3 agents/agent-c/agent.py "Review this code and analyze the text: ..."
"""
...
# ── 1. Validate env vars ──────────────────────────────────────────────────
PRIVATE_KEY = os.environ.get("AGENT_C_EVM_PRIVATE_KEY")
if not PRIVATE_KEY:
    print("ERROR: AGENT_C_EVM_PRIVATE_KEY is not set")
    sys.exit(1)
...
AXL_API = "http://127.0.0.1:9022"  # Node C's API port

print("=" * 60)
print("  AgentMesh — Agent C (Autonomous, Gemini-Powered)")
print("=" * 60)
...
try:
    ...
    print(f"ERROR: Cannot reach AXL Node C at {AXL_API}: {e}")
    sys.exit(1)
...
# ── 5. Known service names to probe ──────────────────────────────────────
...
# ── 7. Discover tools from all peers ─────────────────────────────────────
...
DEFAULT_GOAL = (
    "I need a comprehensive report. First, ask Agent A to analyze this text: "
    "'Decentralized AI agents will reshape how software is built and monetized. "
    "They operate autonomously, pay for services they need, and earn by providing services.' "
    "Then extract keywords and get the sentiment from Agent B's tools. "
    "Finally, rewrite the text in a technical style."
)

user_goal = " ".join(sys.argv[1:]) if len(sys.argv) > 1 else DEFAULT_GOAL

SYSTEM_INSTRUCTION = (
    "You are Agent C, an autonomous AI agent in the AgentMesh network. "
...
print("=" * 60)
print(f"  ✓ Agent C completed — {_call_counter} paid MCP calls made")
