#!/usr/bin/env bash
# ============================================================================
# start-agent-c.sh — Start Agent C's MCP service + MCP Router + A2A Server
#
# Usage: bash scripts/start-agent-c.sh
...
# Validate
if [ -z "${AGENT_C_WALLET_ADDRESS:-}" ]; then
    echo "ERROR: AGENT_C_WALLET_ADDRESS is not set"
    exit 1
fi
...
ROUTER_PORT=9023
A2A_PORT=9024
SERVICE_NAME="agent-c"

echo "============================================"
echo "  AgentMesh — Starting Agent C Services"
echo "============================================"
echo ""
echo "  x402 pay-to wallet: $AGENT_C_WALLET_ADDRESS"

# 1. Start Agent C Flask MCP server
echo ""
echo "[1/3] Starting Agent C MCP service on port $AGENT_C_PORT..."

cd "$AGENT_C_DIR"
python3 server.py &
AGENT_C_PID=$!
echo "       PID: $AGENT_C_PID"

echo "       Waiting for server to start..."
for i in $(seq 1 15); do
    if curl -s http://127.0.0.1:$AGENT_C_PORT/health > /dev/null 2>&1; then
        echo "       ✓ Agent C server is running"
        break
    fi
    if [ $i -eq 15 ]; then
        echo "       ✗ Agent C server failed to start"
...
REGISTER_RESPONSE=$(curl -s -X POST http://127.0.0.1:$ROUTER_PORT/register \
    ...
    -d "{\"service\": \"$SERVICE_NAME\", \"endpoint\": \"http://127.0.0.1:$AGENT_C_PORT/mcp\"}")
...
echo ""
echo "============================================"
echo "  ✓ Agent C services are ready!"
echo "============================================"
echo ""
echo "  Agent C MCP:    http://127.0.0.1:$AGENT_C_PORT/mcp"
echo "  MCP Router:     http://127.0.0.1:$ROUTER_PORT"
echo "  Service name:   $SERVICE_NAME"
echo ""
echo "  PIDs: Agent C=$AGENT_C_PID  Router=$ROUTER_PID"
echo ""
